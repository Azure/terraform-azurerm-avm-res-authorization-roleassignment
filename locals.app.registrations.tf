locals {
  ...
  app_registrations_by_client_id_client_id = { for key, value in var.app_registrations_by_client_id :
    key => value
  }
  app_registrations_by_display_name_client_id = { for key, value in var.app_registrations_by_display_name :
    key => data.azuread_application.applications_by_display_name[key].client_id
  }
  app_registrations_by_object_id_client_id = { for key, value in var.app_registrations_by_object_id :
    key => data.azuread_application.applications_by_object_id[key].client_id
  }
