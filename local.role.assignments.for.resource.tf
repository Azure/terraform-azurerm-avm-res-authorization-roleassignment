locals {
  role_definition_id_prefix = "/subscriptions/${data.azurerm_client_config.current.subscription_id}"
      ...
      for key, value in var.role_assignments_by_resource : [
        for assignment_key, assignment_value in value.role_assignments : [
          for user in assignment_value.users : {
            ...
            role_definition_id = "${local.role_definition_id_prefix}${local.role_definitions[assignment_value.role_definition].id}"
      ...
      for key, value in var.role_assignments_by_resource : [
        for assignment_key, assignment_value in value.role_assignments : [
          for user_assigned_managed_identity in assignment_value.user_assigned_managed_identities : {
            key                = "resource-group-${key}-${assignment_key}-${user_assigned_managed_identity}"
            role_definition_id = "${local.role_definition_id_prefix}${local.role_definitions[assignment_value.role_definition].id}"
